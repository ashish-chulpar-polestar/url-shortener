...
`ac1_latestScores_returns200WithImo9123456()` — Call `restTemplate.getForEntity("/api/v1/vessels/9123456/mti-scores", String.class)`. Assert `response.getStatusCode()` is `HttpStatus.OK`. Parse body as JSON. Assert `$.data.imo_number` equals `"9123456"`. Assert `$.data.year` equals `2024`. Assert `$.data.month` equals `1`.
