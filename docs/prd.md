# MTI Scores API - Product Requirements Document
...
## 7. Database Query Logic

### Latest Scores (No filters)
```sql
SELECT * FROM mti_scores_history
WHERE imo_number = ?
...
1. **UUID Generation**: Use `uuid.v4()` or database UUID function for request_id
2. **Timestamp**: Use ISO 8601 format (UTC)
3. **IMO Validation**: Regex pattern `^[0-9]{7}$`
4. **NULL Handling**: Return `null` in JSON for NULL database values
5. **Logging**: Log request_id with all operations for traceability
6. **Performance**: Add index on `(imo_number, year DESC, month DESC)` for optimal query performance
