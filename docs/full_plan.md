...
The MTI Scores API is a read-only REST service that returns Maritime Transportation Indicator (MTI) scores for a vessel identified by its 7-digit IMO number, with optional filtering by year and/or year+month. The implementation queries a single PostgreSQL table `mti_scores_history` using one of three distinct SQL strategies (latest record, latest-by-year, specific-month) and returns a consistent JSON envelope containing a `meta` block (UUID request ID, ISO 8601 timestamp) and a `data` block (scores with nullable fields). The stack is Spring Boot 3.2 / Java 17 / JdbcTemplate with Flyway migrations, Bucket4j rate limiting, and Testcontainers-based integration tests.
...
|---|---|---|
| Language | Java 17 | LTS release; strong typing for nullable BigDecimal score fields; enterprise maritime ecosystem standard |
| Build | Maven 3.9 | Stable dependency management with Spring Boot parent POM; reproducible CI builds |
| Framework | Spring Boot 3.2 | Auto-configured JDBC, validation, actuator, and servlet filter ordering; jakarta.* namespace |
| Database | PostgreSQL 15 | NUMERIC(5,2) for exact decimal scores; robust NULL handling; strong index support |
| Migrations | Flyway 9 | Version-controlled schema applied on startup; integrated with Spring Boot auto-configuration |
| Rate Limiting | Bucket4j 8.10 + Caffeine 3.1 | In-memory per-IP token bucket; no external dependency required |
| API Docs | springdoc-openapi 2.3 | Auto-generates OpenAPI 3.0 spec from Spring MVC annotations |
| Testing | JUnit 5 + Mockito + Testcontainers | Full unit and integration coverage against a real PostgreSQL container |
| Containerization | Docker (multi-stage) + docker-compose | Reproducible local development environment |
...
└── source/
    ├── src/
    │   ├── main/
    │   │   ├── java/com/maritime/mti/
    │   │   │   ├── MtiScoresApplication.java
    │   │   │   ├── config/
    │   │   │   │   └── RateLimitConfig.java
    │   │   │   ├── controller/
    │   │   │   │   └── VesselMtiScoresController.java
    │   │   │   ├── service/
    ...
    │   │   │   │   └── MtiScoreRecord.java
    │   │   │   ├── dto/
    │   │   │   │   ├── MtiScoresResponse.java
    │   │   │   │   ├── VesselScoresData.java
    │   │   │   │   ├── ErrorResponse.java
    │   │   │   │   ├── ErrorDataDto.java
    │   │   │   │   ├── MetaDto.java
    │   │   │   │   ├── ScoresDto.java
    │   │   │   │   └── RecordMetadataDto.java
    │   │   │   ├── filter/
    ...
    │       ├── java/com/maritime/mti/
    │       │   ├── controller/
    │       │   │   └── VesselMtiScoresControllerIntegrationTest.java
    │       │   ├── service/
...
- **PostgreSQL 15**: single table `mti_scores_history` accessed via Spring JdbcTemplate with positional `?` parameterized queries (SQL injection prevention)
- **No external APIs or message queues**: the service is a read-only REST API over a relational database

### Technical Constraints

- IMO number must match regex `^[0-9]{7}$` — enforced by service layer (ERR_103)
- Year range 2000–2100 enforced by service (ERR_104); month range 1–12 enforced by service (ERR_104)
- Month without year is rejected with ERR_102 before any DB call
- All six score fields are nullable: DB NULL must serialize as JSON `null` (not omitted)
- All responses include `meta.request_id` (UUID v4) and `meta.request_timestamp` (ISO 8601 UTC)
- Rate limit: 100 requests per minute per client IP address
- Index `(imo_number, year DESC, month DESC)` required for acceptable query performance
...
|---|---|---|---|
| `MetaDto` | `requestId: String`, `requestTimestamp: Instant` | `request_id`, `request_timestamp` | Present in every response (success and error) |
| `ScoresDto` | `mtiScore`, `vesselScore`, `reportingScore`, `voyagesScore`, `emissionsScore`, `sanctionsScore`: all `BigDecimal` nullable | `mti_score`, `vessel_score`, `reporting_score`, `voyages_score`, `emissions_score`, `sanctions_score` | Null fields serialize as JSON `null` (not omitted) |
| `RecordMetadataDto` | `createdAt: Instant`, `updatedAt: Instant` | `created_at`, `updated_at` | ISO 8601 UTC strings |
| `VesselScoresData` | `imoNumber: String`, `year: int`, `month: int`, `scores: ScoresDto`, `metadata: RecordMetadataDto` | `imo_number`, `year`, `month`, `scores`, `metadata` | Success response `data` block |
| `MtiScoresResponse` | `meta: MetaDto`, `data: VesselScoresData` | `meta`, `data` | Top-level success body |
| `ErrorDataDto` | `errorCode: String`, `title: String`, `message: String` | `error_code`, `title`, `message` | Error response `data` block |
| `ErrorResponse` | `meta: MetaDto`, `data: ErrorDataDto` | `meta`, `data` | Top-level error body |
| `MtiScoreRecord` | `id: Long`, `imoNumber: String`, `year: int`, `month: int`, `mtiScore–sanctionsScore: BigDecimal nullable`, `createdAt: Instant`, `updatedAt: Instant` | N/A (internal model) | Direct mapping of `mti_scores_history` row |

### Environment Variables Required

| Variable | Required? | Example Value | Description |
|---|---|---|---|
| `DATABASE_URL` | Yes | `jdbc:postgresql://localhost:5432/mti_scores` | JDBC connection URL for PostgreSQL |
| `DATABASE_USER` | Yes | `mti_user` | PostgreSQL username |
| `DATABASE_PASSWORD` | Yes | `mti_pass` | PostgreSQL password |
| `PORT` | No | `8080` | HTTP server port (default 8080) |
...
| `imo_number` | VARCHAR(7) | No | Part of unique constraint `uq_mti_imo_year_month` |
| `year` | INTEGER | No | Check `chk_year_range`: 2000–2100; part of unique constraint |
| `month` | INTEGER | No | Check `chk_month_range`: 1–12; part of unique constraint |
| `mti_score` | NUMERIC(5,2) | Yes | Overall MTI score |
| `vessel_score` | NUMERIC(5,2) | Yes | Vessel sub-score |
| `reporting_score` | NUMERIC(5,2) | Yes | Reporting sub-score |
| `voyages_score` | NUMERIC(5,2) | Yes | Voyages sub-score |
| `emissions_score` | NUMERIC(5,2) | Yes | Emissions sub-score |
| `sanctions_score` | NUMERIC(5,2) | Yes | Sanctions sub-score |
| `created_at` | TIMESTAMP WITH TIME ZONE | No | Default `NOW()` |
| `updated_at` | TIMESTAMP WITH TIME ZONE | No | Default `NOW()` |

Additional constraints:
- Unique constraint `uq_mti_imo_year_month` on `(imo_number, year, month)`
- Index `idx_mti_scores_imo_year_month` on `(imo_number, year DESC, month DESC)`
- Check constraint `chk_month_range`: `month >= 1 AND month <= 12`
- Check constraint `chk_year_range`: `year >= 2000 AND year <= 2100`
...
|------|-----------|--------|------------|
| `rs.getDouble()` used instead of `rs.getBigDecimal()` causes NPE on NULL score columns | M | H | `ROW_MAPPER` explicitly uses `rs.getBigDecimal()` for all six score columns; code review to enforce |
| Jackson serializes `Instant` as epoch array instead of ISO 8601 string | M | H | `spring.jackson.serialization.write-dates-as-timestamps=false` set in `application.yml`; verified by integration tests |
| Flyway migration fails on re-deploy against existing schema | L | H | Flyway checksums detect tampering; use `flyway.validate-on-migrate=true` (default); never edit a deployed migration |
| Testcontainers requires Docker daemon in CI environment | M | M | Ensure CI has Docker available; Testcontainers will skip gracefully if Docker is unavailable when `assumeDockerAvailable()` is used |
| Caffeine bucket cache grows unbounded under unique IP attack | L | M | `maximumSize(10000)` and `expireAfterAccess(1h)` set in `RateLimitConfig`; cap enforced at JVM level |
| SQL injection via path/query parameters | L | H | All three queries use positional `?` parameters with JdbcTemplate; no string concatenation in SQL anywhere |
| Month-without-year validation skipped (wrong order in service) | L | M | Validation order documented and tested: IMO check first, cross-param check second, range check third |
...
- [ ] Performance targets verified under load (p95 < 50ms at 100 concurrent)
- [ ] Security review passed: parameterized queries confirmed, no stack traces in responses, rate limiting active
- [ ] API documentation accessible at `/swagger-ui.html`
- [ ] Docker image builds and `docker-compose up` starts the full stack locally
...
1. **Story 1.1** — project skeleton; everything depends on a compiling build
2. **Story 1.2** — database schema; required before repository code or tests
3. **Story 2.1** — `MtiScoreRecord`; required by repository
4. **Story 2.2** — `MtiScoresRepository`; foundational data access
5. **Story 3.1** — `ErrorCode` + `MtiException`; no dependencies; unblocks service and handler
6. **Story 4.1** — `RequestIdFilter` + `REQUEST_ID_ATTR` constant; referenced by controller, handler, rate filter
7. **Story 3.2** — `GlobalExceptionHandler`; imports `REQUEST_ID_ATTR` from Story 4.1
8. **Story 4.2** — `RateLimitFilter`; depends on Story 4.1 for filter ordering and constant
9. **Story 5.1** — response DTOs; required by controller
10. **Story 5.2** — `MtiScoresService`; depends on repository (2.2) and error codes (3.1)
11. **Story 5.3** — `VesselMtiScoresController`; final wiring of all components
12. **Story 6.1** — service unit tests (can run in parallel with Story 6.2)
13. **Story 6.2** — repository integration tests (can run in parallel with Story 6.1)
14. **Story 6.3** — full-stack controller tests; needs complete application (after Story 5.3)
15. **Story 7.1** — containerization; independent of Epic 6; can proceed after Story 1.1

> Stories 6.1 and 6.2 can be developed in parallel. Story 7.1 is independent and can proceed as soon as Story 1.1 is complete.
