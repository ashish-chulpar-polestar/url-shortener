...
Implement method `findLatest(String imoNumber)` returning `Optional<MtiScoreRecord>`. Log at DEBUG: `log.debug("findLatest imo={}", imoNumber)`. Execute the SQL `SELECT * FROM mti_scores_history WHERE imo_number = ? ORDER BY year DESC, month DESC LIMIT 1` with parameter array `new Object[]{imoNumber}` using `jdbcTemplate.query(sql, ROW_MAPPER, imoNumber)`. Return the first element wrapped in `Optional.ofNullable`, or `Optional.empty()` if the result list is empty.
